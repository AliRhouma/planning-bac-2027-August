# Planning des séances — Bac 2026

Répartition des séances de révision du **19 août au 12 septembre 2026** : 121 séances,
17 professeurs, 9 groupes répartis sur 6 sections (Sc, Math, Tech, Lettres, Éco, Info).

## Contenu

| Fichier | Description |
|---|---|
| [`index.html`](index.html) | Planning complet à onglets : calendrier global, par bac, par groupe, par professeur, méthode et contrôles. Page autonome, à ouvrir dans un navigateur. |
| [`Calender/emploi-du-temps-repartition.md`](Calender/emploi-du-temps-repartition.md) | Même planning au format texte : calendrier maître, emploi du temps de chaque groupe et de chaque professeur, tableaux de contrôle. |
| [`Calender/professeurs-par-matiere.md`](Calender/professeurs-par-matiere.md) | Données sources : volume de séances par professeur et par section, règles de créneaux. |
| [`emploi-du-temps.html`](emploi-du-temps.html) | Outil d'édition d'emploi du temps existant. |
| [`groupes-par-niveau.html`](groupes-par-niveau.html), [`niveaux.html`](niveaux.html) | Pages annexes. |

## Règles de construction

1. **Une séance par jour et par groupe**, à 11h – 13h ou 14h – 16h. Aucun trou dans la journée.
2. **Au maximum 2 jours vides consécutifs** pour un groupe — 8 groupes sur 9 n'ont jamais plus d'un jour sans cours.
3. **Le professeur enchaîne ses deux groupes le même jour** : Élite à 11h, Intermédiaire à 14h. Jamais plus de 4 h par jour.
4. **Alternance matin / après-midi** entre les deux niveaux d'une même section.
5. **Rotation des professeurs** dans chaque section, pour que chaque matière revienne à intervalle régulier.
6. **Faycal Hdiri** (Sc + Math) et **Mahdi Ghanmi** (Tech + Info) ont des jours strictement séparés entre leurs deux sections.

## Contraintes respectées

- Repos hebdomadaire le **dimanche** ; les samedis sont travaillés. Samedi 12/09 laissé libre comme journée tampon.
- **5 salles** (hôtes Zoom) disponibles, jamais dépassées.
- Créneau **17h – 19h jamais utilisé**, gardé en réserve pour les rattrapages.
- Volume de séances respecté à la séance près pour les 17 professeurs.
- 0 conflit de professeur, 0 conflit de groupe.
